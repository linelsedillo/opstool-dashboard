...
module.exports = {
    ...
    command:function(builder, cb) {
        ...
        // build command:  ./cjs steal/buildjs OpsPortal opstools/Dashboard
        ...
        async.series([
            ...
            function(next){
                ...
                AD.log('<green>building</green> opstools/Dashboard');

                AD.spawn.command({
                    command:'./cjs',
                    // options:[path.join('opstools', 'Dashboard', 'build.js')],
                    options:[path.join('steal', 'buildjs'), 'OpsPortal', path.join('opstools', 'Dashboard')],
shouldEcho:true,
                    // exitTrigger:'opstools/Dashboard/production.css'
                })
                .fail(function(err){
                    AD.log.error('<red>could not complete opstools/Dashboard build!</red>');
                    next(err);
                })
                .then(function(){
            ...
            function(next) {
                ...
                var patches = [
                    { file:path.join('opstools', 'Dashboard', 'production.js'), tag:'packages/OpsPortal-Dashboard.js', replace:'OpsPortal/production.js'}
                ];
